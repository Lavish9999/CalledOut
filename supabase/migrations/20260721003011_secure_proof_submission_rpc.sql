...
begin
  ...
  if p_captured_at < v_commitment.proof_window_starts_at
     or p_captured_at >
        v_commitment.deadline_at
        + make_interval(mins => v_commitment.grace_period_minutes)
  then
    raise exception 'Proof was captured outside the allowed proof window'
      using errcode = '22023';
  end if;
...
revoke insert on table public.proof_submissions
