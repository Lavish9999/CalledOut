...
select m.circle_id,m.user_id,min(m.id::text)::uuid as id,count(*)::int as missed_count,max(m.missed_at) as most_recent_missed_at,p.completion_rate,p.display_name,p.username,p.avatar_path,
